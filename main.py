...
MAKE_MINI_DOCS = True
Q_EPS = 1e-6  # numerical tolerance for Q ties
...
THEME_DARK = {
    ...
    "accent": (86, 140, 255),
    "button": (45, 50, 58),
    "button_active": (86, 140, 255),
    "cell_border": (58, 64, 72)
}
THEME_LIGHT = {
    "bg": (245, 247, 250),
    "grid": (220, 224, 230),
...
def greedy_set(env: GridWorld, V, s) -> List[Action]:
    ...
    return [a for a, q in qs if q >= max_q - 1e-9]  # tolerant tie
    ...
    for y in range(env.height):
        ...
        for x in range(env.width):
            ...
            if s in env.walls:
                ...
                row.append("G+" if env.terminals[s]>0 else "G-")
            elif s==env.start:
                row.append("S ")
            else:
                row.append(ARROWS.get(policy.get(s), "· "))
...
def print_episode_header(mode, env: GridWorld, V, policy):
    print("\n────────────────────────────────────────────────────────")
    print(f"Episode start — Current Mode: {mode}")
    print(f"γ = {env.gamma} | step_reward={env.step_reward:+.2f} | slip={env.slip_prob*100:.0f}% | terminals={env.terminals}")
...
def print_q_table_for_state(env: GridWorld, V, s, policy):
    ...
    ties = [a for a, q in qs if q >= max_q - 1e-9]
    if len(ties) > 1:
        print(f" Greedy tie set: {stringify_actions(ties)} (|Δ| ≤ {1e-9})")
    return ties
...
class Button:
    ...
    def draw(self, surface, fonts, T):
        ...
        if ft is not None:
            w=fonts["ui"].get_rect(self.label).width; fonts["ui"].render_to(surface,(self.rect.centerx-w//2,self.rect.y+12),self.label,(255,255,255) if (self.active or self.accent) else T["text"])
        else:
            img=fonts["ui"].render(self.label,True,(255,255,255) if (self.active or self.accent) else T["text"]); surface.blit(img,(self.rect.centerx-img.get_width()//2,self.rect.y+(self.rect.height-img.get_height())//2))
    def handle(self,pos):
        return self.value if self.rect.collidepoint(pos) else None

class Toggle:
    def __init__(self, rect, value=False):
        self.rect=pygame.Rect(rect); self.value=value
    def draw(self,surface,fonts,T,label,x_label):
        ...
        if ft is not None:
            fonts["ui"].render_to(surface,(x_label,self.rect.y+self.rect.height//2-10),label,T["text"])
        else:
            img=fonts["ui"].render(label,True,T["text"]); surface.blit(img,(x_label,self.rect.y+self.rect.height//2-10))
    def handle(self,pos):
        if self.rect.collidepoint(pos):
            self.value=not self.value; return self.value
        return None
...
class Slider:
    ...
    def draw(self,surface,fonts,T,label,x,width,suffix=""):
        ...
        if ft is not None:
            fonts["ui"].render_to(surface,(x,self.rect.y-26),title,T["text"])
        else:
            img=fonts["ui"].render(title,True,T["text"]); surface.blit(img,(x,self.rect.y-26))
        ...
        pygame.draw.rect(surface,(86,140,255),pygame.Rect(kx,self.rect.y,self.knob_w,self.rect.height),border_radius=8)
    def handle(self,pos):
        if self.rect.collidepoint(pos):
...
class Visualizer:
    def __init__(self, env, cell=CELL_SIZE, margin=MARGIN, sidebar_w=SIDEBAR_W):
        ...
        # tabs & controls
        self.tabs=[Button((self.grid_w+20,12,110,36),"Modes","Modes"), Button((self.grid_w+140,12,120,36),"Settings","Settings"), Button((self.grid_w+270,12,140,36),"Instructions","Instructions")]
    ...
    def apply_settings(self):
        ...
        if self.toggle_trap.value:
            self.env.terminals[(0,4)] = -1.0
        else:
            self.env.terminals.pop((0,4), None)
        # recompute
        self.V_vi,self.pi_vi=value_iteration(self.env)
        self.V_pi,self.pi_pi=policy_iteration(self.env)
    ...
    def step_log(self, a, ns, r):
        ...
        if not self.env.is_terminal(self.s):
            ...
            max_q = max(q for _, q in qs) if qs else 0.0
            ties = [act for act, q in qs if q >= max_q - 1e-9]
            if len(ties) > 1:
                print(f" Greedy tie set: {stringify_actions(ties)}")
    ...
    def draw_grid(self):
        ...
        for y in range(self.env.height):
            for x in range(self.env.width):
                ...
                pygame.draw.rect(self.screen,(33,37,43) if self.theme is THEME_DARK else (255,255,255),rect,border_radius=10)
                pygame.draw.rect(self.screen,THEME_DARK["cell_border"] if self.theme is THEME_DARK else THEME_LIGHT["cell_border"],rect,width=1,border_radius=10)
                if (x,y) in self.env.walls:
                    pygame.draw.rect(self.screen,(90,100,112) if self.theme is THEME_DARK else (80,92,104),rect,border_radius=10)
                if (x,y) in self.env.terminals:
                    col = (36,94,64) if self.env.terminals[(x,y)]>0 else (140,42,48)
        ...
        pygame.draw.rect(self.screen,(86,140,255),pygame.Rect(srx,sry,self.cell,self.cell),width=3,border_radius=10)
        # agent
        ax,ay=self.s; cx=ax*self.cell+(ax+1)*self.margin+self.cell//2; cy=ay*self.cell+(ay+1)*self.margin+self.cell//2
        r=max(12,self.cell//3); gfxdraw.filled_circle(self.screen,cx,cy,r,(86,140,255)); gfxdraw.aacircle(self.screen,cx,cy,r,(86,140,255))
        if SHOW_POLICY_ARROW_ABOVE_AGENT and self.mode in ('VI','PI') and not self.env.is_terminal(self.s):
            a=self.policy.get(self.s)
            if a is not None:
                if ft is not None:
                    self.fonts["title"].render_to(self.screen,(cx-8,cy-r-28),ARROWS[a],(86,140,255))
                else:
                    img=self.fonts["title"].render(ARROWS[a],True,(86,140,255)); self.screen.blit(img,(cx-8,cy-r-28))

    def draw_sidebar(self):
        px=self.grid_w+10; py=10
        panel=pygame.Surface((self.sidebar_w-20,self.grid_h-20), pygame.SRCALPHA)
        pygame.draw.rect(panel,(28,32,38,235),panel.get_rect(),border_radius=16)
        self.screen.blit(panel,(px,py))
        # tabs
        for b in self.tabs:
            b.active=(b.value==self.active_tab); b.draw(self.screen,self.fonts,THEME_DARK)
        x=self.grid_w+24; w=self.sidebar_w-48; y=86
        self.update_highlights()
        if self.active_tab=="Modes":
            for btn in (self.btn_vi,self.btn_pi,self.btn_rand,self.btn_auto):
                btn.rect.topleft=(x,y); btn.rect.width=w; btn.draw(self.screen,self.fonts,THEME_DARK); y+=56 if btn!=self.btn_auto else 64
            for i,t in enumerate([f"Pos: {self.s}", f"G_t (discounted): {self.G:+.2f}", f"Auto budget: {self.auto_budget}/{self.auto_max}", f"Mode: {self.mode}"]):
                if ft is not None:
                    self.fonts["ui"].render_to(self.screen,(x,y+i*26),t,(230,235,240))
                else:
                    img=self.fonts["ui"].render(t,True,(230,235,240)); self.screen.blit(img,(x,y+i*26))
        elif self.active_tab=="Settings":
            self.toggle_theme.rect.topleft=(x,y); self.toggle_theme.draw(self.screen,self.fonts,THEME_DARK,"Dark theme",x+96); y+=60
            self.slider_budget.rect.y=y; self.slider_budget.draw(self.screen,self.fonts,THEME_DARK,"Auto-step budget",x,w); y+=60
            self.slider_slip.rect.y=y; self.slider_slip.draw(self.screen,self.fonts,THEME_DARK,"Slip probability",x,w,suffix="%"); y+=60
            self.toggle_trap.rect.topleft=(x,y); self.toggle_trap.draw(self.screen,self.fonts,THEME_DARK,"Enable trap goal at (0,4) with reward -1.0",x+96); y+=40
            apply_btn = Button((x,y,180,36),"Apply settings","APPLY",accent=True)
            apply_btn.draw(self.screen,self.fonts,THEME_DARK)
            ...
            for para in paragraphs:
                for line in wrap(self.fonts["ui"], para, max_w):
                    if ft is not None:
                        self.fonts["ui"].render_to(self.screen,(x,yy),line,(230,235,240))
                    else:
                        img=self.fonts["ui"].render(line,True,(230,235,240)); self.screen.blit(img,(x,yy))
                    yy+=22
                yy+=4
            ...
            for b in (self.btn_vi,self.btn_pi,self.btn_rand,self.btn_auto):
                ...
                if val is not None:
                    if val=="AUTO":
                        ...
                        if self.auto:
                            self.auto_budget=self.auto_max; self.next_step_time_ms=pygame.time.get_ticks()+STEP_DELAY_MS
                        if not self.header_printed:
                            self._episode_header_cli(); self.header_printed=True
                        return
                    self.set_mode(val); return
        elif self.active_tab=="Settings":
            if self.toggle_theme.handle(pos) is not None:
                pass
            if self.slider_budget.handle(pos) is not None:
                self.auto_max=int(self.slider_budget.value); self.auto_budget=min(self.auto_budget,self.auto_max)
            if self.slider_slip.handle(pos) is not None:
                pass
            if self.toggle_trap.handle(pos) is not None:
                pass
            if hasattr(self,"_apply_button"):
                if self._apply_button.handle(pos)=="APPLY":
                    self.apply_settings()
