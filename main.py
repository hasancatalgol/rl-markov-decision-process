...
class Button:
    ...
    def draw(self, surface, fonts, T):
        ...
        if ft is not None: w=fonts["ui"].get_rect(self.label).width; fonts["ui"].render_to(surface,(self.rect.centerx-w//2,self.rect.y+12),self.label,(255,255,255) if (self.active or self.accent) else T["text"])
        else: img=fonts["ui"].render(self.label,True,(255,255,255) if (self.active or self.accent) else T["text"]); surface.blit(img,(self.rect.centerx-img.get_width()//2,self.rect.y+(self.rect.height-img.get_height())//2))
    ...
    def draw_grid(self):
        ...
        for y in range(self.env.height):
            for x in range(self.env.width):
                ...
                if (x,y) in self.env.walls: pygame.draw.rect(self.screen,(90,100,112) if self.theme is THEME_DARK else (80,92,104),rect,border_radius=10)
        ...
        if SHOW_POLICY_ARROW_ABOVE_AGENT and self.mode in ('VI','PI') and not self.env.is_terminal(self.s):
            ...
            if a is not None:
                if ft is not None: self.fonts["title"].render_to(self.screen,(cx-8,cy-r-28),ARROWS[a],(86,140,255))
                else: img=self.fonts["title"].render(ARROWS[a],True,(86,140,255)); self.screen.blit(img,(cx-8,cy-r-28))
    ...
    def draw_sidebar(self):
        ...
        if self.active_tab=="Modes":
            ...
            for i,t in enumerate([f"Pos: {self.s}", f"G_t (discounted): {self.G:+.2f}", f"Auto budget: {self.auto_budget}/{self.auto_max}", f"Mode: {self.mode}"]):
                if ft is not None: self.fonts["ui"].render_to(self.screen,(x,y+i*26),t,(230,235,240))
                else: img=self.fonts["ui"].render(t,True,(230,235,240)); self.screen.blit(img,(x,y+i*26))
            ...
            for para in paragraphs:
                for line in wrap(self.fonts["ui"], para, max_w):
                    if ft is not None: self.fonts["ui"].render_to(self.screen,(x,yy),line,(230,235,240))
                    else: img=self.fonts["ui"].render(line,True,(230,235,240)); self.screen.blit(img,(x,yy))
                    yy+=22
                yy+=4
